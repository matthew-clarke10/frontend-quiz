...
const Layout: React.FC<LayoutProps> = ({ toggleMusicEnabled, musicEnabled }) => {
  ...
  return (
    <>
      <div className="flex justify-between items-center h-header-small md:h-header bg-blue-500">
        {quizPage && (
        ...
        <h1 className={`flex justify-center items-center bg-blue-500 w-full font-bold text-center ${quizPage ? 'text-xl sm:text-3xl md:text-5xl' : 'text-5xl'}`}>{quizPage ? `Quiz: ${headerText}` : 'Front-end Quizzes'}</h1>
        {quizPage && (
